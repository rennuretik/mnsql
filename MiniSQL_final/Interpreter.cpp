...
int Interpreter::interpreter(string s)
{
	...
	if (strcmp(word.c_str(), "create") == 0)
	{
		...
		if (strcmp(word.c_str(), "table") == 0)
		{
			...
			else				// deal with attribute list
			{
				...
				while (!word.empty() && strcmp(word.c_str(),"primary") != 0 && strcmp(word.c_str(),")") != 0)
				{
					...
					Attribute attr(attributeName,type,if_unique);
					...
					else
					{
						...
						if (strcmp(word.c_str(),"(") == 0)
						{
							...
							for( i= 0;i<attributeVector.size();i++)
							{
								...
								{
									attributeVector[i].if_unique = true;
									break;
								}
				...
				ap->create_table(tableName,&attributeVector,primaryKey,primaryKeyLocation);
				return 1;
			}
		}
		else if (strcmp(word.c_str(), "index") == 0)
		{
			...
			try {
				...
				ap->create_index(indexName,tableName,attributeName);
				return 1;
	...
	else if(strcmp(word.c_str(), "select")==0)
	{
		...
		if (word.empty())	// without condition
		{
			if(attrSelected.size()==0){
				ap->show_record(tableName);
			}
			else
				ap->show_record(tableName,&attrSelected);
			return 1;
		}
		else if (strcmp(word.c_str(),"where") == 0)
		{
			...
			if(attrSelected.size()==0)
				ap->show_record(tableName,NULL,&conditionVector);
			else
				ap->show_record(tableName, &attrSelected,&conditionVector);
	...
	else if (strcmp(word.c_str(), "delete")==0)
	{
		...
		else if (strcmp(word.c_str(),"where") == 0)
		{
			...
			ap->delete_record(tableName, &conditionVector);
			return 1;
		}
	}
	else if (strcmp(word.c_str(), "insert") == 0)
	{
		...
		ap->insert_record(tableName,&valueVector);
		return 1;
	}
...
string Interpreter::getWord(string s, int *tmp)
{
	...
	while ((s[*tmp] == ' ' || s[*tmp] == 10  || s[*tmp] == '\t' || s[*tmp] == '\r') && s[*tmp] != 0)
	{
		(*tmp)++;
	}
	idx1 = *tmp;
